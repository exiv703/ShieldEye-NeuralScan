...
class SecurityScanner:
    ...
    def prepare_ai_analyzer(self, model_name: Optional[str] = None) -> None:
    ...
    def _adjust_severity_by_context(self, finding: Dict[str, Any], lines: List[str], line_idx: int) -> Dict[str, Any]:
        ...
        # Only honour a suppression marker on the finding's own line, never elsewhere in the context window.
    ...
    def _fallback_explanation(self, description: str, code_snippet: str, category: Optional[str] = None) -> str:
    ...
    def scan_file_for_malware(self, file_path: str, custom_prompt: Optional[str] = None, detail: Optional[str] = None) -> Dict[str, Any]:
        ...
        # Reject oversized files so a huge upload can't exhaust memory.
        ...
        if self.settings.get('use_trivy'):
            # Spin up Docker lazily, only when a scan actually needs Trivy.
        ...
        try:
            ...
            # Effective detail level: per-call override beats the global setting.
            ...
            for idx, d in enumerate(detected, start=1):
                ...
                try:
                    ...
                    line_idx = max(0, int(d['line']) - 1)
                    ctx_lines = lines[max(0, line_idx - 1): min(len(lines), line_idx + 2)]
                    ctx = "\n".join(ctx_lines)
                except (ValueError, KeyError):
                    ctx = None

                if effective_detail in ('short', 'quick'):
                ...
                try:
                    ...
                    logging.debug("Could not build redacted explanation preview", exc_info=True)
            ...
            if AI_LIBS_AVAILABLE and torch.cuda.is_available():
                try:
                    ...
                    logging.debug("Could not clear GPU memory cache", exc_info=True)
